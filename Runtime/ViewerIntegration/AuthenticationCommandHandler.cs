...
namespace Deucarian.Authentication
{
    ...
    /// authentication host.
    /// </summary>
    public sealed class AuthenticationCommandHandler<THost> :
        ...
        where THost : class, IAuthenticationHost
    {
        ...
        private static readonly IReadOnlyList<string> SupportedNames =
            Array.AsReadOnly(
                new[]
                {
                    ...
                    AuthenticationCommandNames.RefreshAccessToken,
                    AuthenticationCommandNames.ClearAccessToken
                });

        private readonly IAuthenticationEventPublisher eventPublisher;
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                case AuthenticationCommandNames.UpdateAccessToken:
                case AuthenticationCommandNames.UpdateAccessTokenLegacy:
                    ...
                    sessionResult = await session.ReplaceAccessTokenAsync(
                        ...
                        expiresAtUtc,
                        cancellationToken);
                    eventName = AuthenticationEventNames.AccessTokenUpdated;
                    break;

                case AuthenticationCommandNames.RefreshAccessToken:
                    sessionResult =
                        await session.RefreshAsync(cancellationToken);
                    eventName = AuthenticationEventNames.AccessTokenRefreshed;
                    break;

                case AuthenticationCommandNames.ClearAccessToken:
                    sessionResult =
                        await session.ClearAsync(cancellationToken);
                    eventName = AuthenticationEventNames.AccessTokenCleared;
                    break;
            ...
            AuthenticationStatusSnapshot status = session.Status;
